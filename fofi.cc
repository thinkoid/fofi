...
advance (Iterator& iter, Iterator last, T dist) {
    if (std::distance (iter, last) < 0 || size_t (std::distance (iter, last)) < dist)
        iter = last;
    else
        std::advance (iter, dist);
}
...
identify_pfa (Iterator& iter, Iterator last, font_type& result) {
...
template< typename Iterator >
typename std::enable_if_t< is_random_access_iterator< Iterator >, bool >
identify_pfb (Iterator& iter, Iterator last, font_type& result) {
    ITERATOR_GUARD (iter);
...
template< typename Iterator >
typename std::enable_if_t< is_random_access_iterator< Iterator >, bool >
identify_ttf (Iterator& iter, Iterator last, font_type& result) {
...
template< typename Iterator >
typename std::enable_if_t< is_random_access_iterator< Iterator >, bool >
identify_cff (Iterator& iter, Iterator last, font_type& result) {
    ITERATOR_GUARD (iter);
...

    {
        ...
        if (!integral (iter, last, a) ||
            !integral (iter, last, b))
            return false;

        if (b < 1 || 4 < b || a < 4)
            return false;

        if (!safe_advance (iter, last, int (a) - 4))
            return false;
    }

    {
        ...
        if (n) {
            ...
            if (!safe_advance (iter, last, long (y) - 1))
                return false;
        }
    }

    {
        ...
        for (size_t i = 0; i < 3; ++i) {
            ...
            if (c == 0x1c) {
                if (!safe_advance (iter, last, 2))
                    return false;
            } else if (c == 0x1d) {
                if (!safe_advance (iter, last, 4))
                    return false;
            } else if (c >= 0xf7 && c <= 0xfe) {
                if (!safe_advance (iter, last, 1))
                    return false;
        ...
        if (integral (iter, last, c) && c == 12 &&
            integral (iter, last, c) && c == 30) {
            result = CFFCID;
        }
        else {
            result = CFF8Bit;
...
template< typename Iterator >
typename std::enable_if_t< is_random_access_iterator< Iterator >, bool >
identify_otf (Iterator& iter, Iterator last, font_type& result) {
    ITERATOR_GUARD (iter);
                ...
                if (literal_string (iter, last, "CFF ")) {
                    ...
                    if (integral (iter, last, off)) {
                        ...
                        if (off < INT_MAX) {
                            ...
                            if (!safe_advance (iter2, last, off))
                                break;
...
template< typename Iterator >
typename std::enable_if_t< is_random_access_iterator< Iterator >, bool >
identify (Iterator& iter, Iterator last, font_type& result) {
    return
...
static bool
identify_byextension (const char* filepath, font_type& result) {
    if (fs::path (filepath).extension () == ".dfont") {
        return result = DFont, true;
    }
...
static bool
identify_bycontent (const char* filepath, font_type& result) {
    io::mapped_file_source src (filepath);
    auto iter = src.begin (), last = src.end ();
    return identify (iter, last, result);
}

static bool
identify (const char* filepath, font_type& result) {
    return
        identify_byextension (filepath, result) ||
